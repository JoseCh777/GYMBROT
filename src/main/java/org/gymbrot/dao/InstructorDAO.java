...
public class InstructorDAO {
    ...
    public Instructor buscarPorId(String numeroIdentificacion) {
        ...
        try (PreparedStatement ps = getConexion().prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return mapearInstructor(rs);
            }
        } catch (SQLException e) {
    ...
    public List<Instructor> listarTodos() {
        ...
        try (PreparedStatement ps = getConexion().prepareStatement(sql);
            ...
            while (rs.next()) instructores.add(mapearInstructor(rs));
        } catch (SQLException e) {
    ...
    private Instructor mapearInstructor(ResultSet rs) throws SQLException {
        ...
        if (fechaReg != null) instructor.setFechaRegistro(fechaReg.toLocalDate());
        ...
        if (fechaCont != null) instructor.setFechaContratacion(fechaCont.toLocalDate());
        return instructor;
    }
}